...
//where data should submit to the server
  const submitOrderHandler = async (userData)=>{
   ...
   await fetch("https://foodorder-7968b-default-rtdb.firebaseio.com/orders.json", {
      method : 'POST',
      body : JSON.stringify({
        user : userData,
...
const cartModalContent = <React.Fragment>
  {cartItems}
      ...
      {showCheckout && <CheckoutForm onConfirmOrder={submitOrderHandler}  onCancel = {props.onCloseCartBtn}/>}
...
// is shown we submit the form
  ...
  const didSubmitModalContent =<React.Fragment>
    <p>successfully send the order.., we will notify you once the order is ready.</p>
